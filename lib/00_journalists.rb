...
def nbjournalists(journalists) 
    number = journalists.count
    return number
end

nb = nbjournalists(journalists)
puts "#{nb} : c'est le nombre de paparazzi dans l'array !"

# Combien y a t il de handle (@) dans l'array ?


def smallest(journalists)
    small = journalists.min_by(&:length)
    return small
end

petit = smallest(journalists)
puts "#{petit} est le plus petit @"
